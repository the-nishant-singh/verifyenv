...
const checkAwsSecrets = ({
  ...
  let missingVariablesFromExample = false;
  if (reverseCheck) {
    missingVariablesFromExample =
      Checker.hasExampleMissingVariablesComparedToSecrets(example, secrets);
  }
  ...
  if (missingVariablesFromEnv || missingVariablesFromExample) {
    Message.failMessage();
    if (missingVariablesFromEnv.length) {
      Message.missingMessage(missingVariablesFromEnv, "Secrets");
      errorMessage = errorMessage.concat(
        Message.missingMessageAsString(missingVariablesFromEnv, "Secrets")
      );
    }
    if (missingVariablesFromExample.length) {
      Message.missingMessage(missingVariablesFromExample, example);
...
module.exports = { check, checkAwsSecrets };
